...
def main():
    st.title("Pneumonia Image Diagnostic (Normal/Pneumonia)")
    result = ""
    img_file = st.file_uploader("Please Upload the Patient's Chest X-Ray Image", type=["png", "jpeg"])
    ...
    if img_file is not None:
        ...
        if st.button("See Results"):
            ...
            if pred is not None:
                if pred > 0.5:
                    result = "Patient has PNEUMONIA!"
                else:
                    result = "Patient has NO PNEUMONIA (NORMAL)!"
